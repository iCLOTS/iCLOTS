...
Brightfield deformability analysis GUI analyzes a single brightfield video
of cells transiting the biophysical flow cytometer device
...
import numpy as np
from help import defbrightfieldhelp as hp
from analysis import deform as an
from accessoryfn import chooseinput, error, complete
import datetime


class BrightfieldDeformGUI(tk.Toplevel):
    ...
    def __init__(self):
        ...
        self.img_canvas.grid(row=3, column=2, rowspan=3, padx=5, pady=5, sticky='NSEW')
        self.img_canvas.bind("<Configure>", self.changeparameter_window)
        # Manipulated image canvas
        self.manip_canvas = tk.Canvas(self, width=150, height=500)
        self.manip_canvas.grid(row=3, column=3, rowspan=3, padx=5, pady=5, sticky='NSEW')
    ...
    def expimgs(self):
        ...
        if self.analysisbool.get() is True:
            if self.analysis_exp_bool.get() is True:
                an.RunBFDefAnalysis.expimgs(self, frames_crop)
                complete.DoneWindow()
            else:
