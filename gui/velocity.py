...
class VelocityGUI(tk.Toplevel):
    ...
    def __init__(self):
        ...
        self.manip_canvas.grid(row=2, column=2, rowspan=6, padx=5, pady=5, sticky='NSEW')
    ...
    def changeparameter_window(self, event):
        """As user interactively configures the window, the center display updates"""
    ...
    def resizeimg(self, manip):
        ...
        if width/(wscale*500) > height/(hscale*300):
            factor = (wscale*500) / width
        else:  # If height ratio greater or if equal (if equal factor same)
            factor = (hscale*300)/height
