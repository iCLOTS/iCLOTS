...
from accessoryfn import chooseinput, error, fluor_multi, complete  # Edit
import datetime
...
class MicrochannelOccGUI(tk.Toplevel):
    ...
    def __init__(self):
        ...
        self.img_canvas.grid(row=5, column=2, rowspan=6, padx=5, pady=5, sticky='NSEW')
        self.img_canvas.bind("<Configure>", self.changeparameter_window)
        # Map canvas
        self.map_canvas = tk.Canvas(self, width=150, height=500)
        self.map_canvas.grid(row=5, column=3, rowspan=6, padx=5, pady=5, sticky='NSEW')
        self.map_canvas.bind("<Configure>", self.changeparameter_window)
        ...
        # Control image displayed with arrows, not specific to when scale is active
        ...
        # For calculating change in canvas image
        self.update()
    ...
    def colorchoice(self):
        ...
        if self.rchannel.get() is True:
            ...
            self.red_canvas.grid(row=5, column=4, rowspan=6, padx=5, pady=5, sticky='NSEW')
            self.red_canvas.bind("<Configure>", self.changeparameter_window)
        ...
        if self.gchannel.get() is True:
            ...
            self.green_canvas.grid(row=5, column=5, rowspan=6, padx=5, pady=5, sticky='NSEW')
            self.green_canvas.bind("<Configure>", self.changeparameter_window)
        ...
        if self.bchannel.get() is True:
            ...
            self.blue_canvas.grid(row=5, column=6, rowspan=6, padx=5, pady=5, sticky='NSEW')
            self.blue_canvas.bind("<Configure>", self.changeparameter_window)
    ...
    def changeparameter_window(self, event):
        """As user interactively configures the window size, the center display updates"""
    ...
    def displayimg(self, filename):
        ...
        self.img_canvas.create_image(0, 0, anchor='nw', image=imgrs_tk)
        ...
        self.map_canvas.create_image(0, 0, anchor='nw', image=imgrs_map_tk)
        ...
        if self.rchannel.get() is True:
            ...
            self.red_canvas.create_image(0, 0, anchor='nw', image=imgrs_red_tk)
        ...
        if self.gchannel.get() is True:
            ...
            self.green_canvas.create_image(0, 0, anchor='nw', image=imgrs_green_tk)
