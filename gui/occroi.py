...
Fluorescent occlusion and accumulation analysis in region of interest GUI provides interface
...
import numpy as np
from help import occroihelp as hp  # Edit
from analysis import occroi as an  # Edit
from accessoryfn import chooseinput, error, fluor_multi, complete  # Edit
import datetime


class ROIOccGUI(tk.Toplevel):
    ...
    def __init__(self):
        ...
        # Widgets
        self.title(name + " region of interest accumulation and occlusion analysis")
    ...
    def singlefile(self):
        ...
        self.single_label.config(text=os.path.basename(self.filename))
    ...
    def colorchoice(self):
        ...
        if self.rchannel or self.gchannel or self.bchannel:
            self.map_canvas.grid(row=5, column=3, rowspan=3, padx=5, pady=5, sticky='NSEW')
        else:
    ...
    def displayimg(self, file):
        ...
        if self.rchannel or self.gchannel or self.bchannel:
            ...
            if self.rchannel is True:
                ...
                img_th_red = np.zeros((crop.shape[0], crop.shape[1]))

            if self.gchannel is True:
                ret, img_th_green = cv2.threshold(crop[:, :, 1], self.gthresh.get(), 255, cv2.THRESH_BINARY)  # Green
            else:
                img_th_green = np.zeros((crop.shape[0], crop.shape[1]))

            if self.bchannel is True:
                ret, img_th_blue = cv2.threshold(crop[:, :, 0], self.bthresh.get(), 255, cv2.THRESH_BINARY)  # Blue
            else:
                img_th_blue = np.zeros((crop.shape[0], crop.shape[1]))

            # Combine
            img_map = np.dstack((img_th_red, img_th_green, img_th_blue)).astype('uint8')

            imgrs_map = self.resizeimg(img_map)
    ...
    def runanalysis(self):
        ...
        an.RunOccAccROIAnalysis.analysis(
                    self,
                    self.filelist,
                    self.umpix.get(),
                    self.rchannel,
    ...
    def expall(self):
        ...
        if self.analysisbool.get() is True:
            ...
            # Numerical data
            an.RunOccAccROIAnalysis.expnum(
                    self,
                    self.filelist,
                    self.umpix.get(),
                    self.rchannel,
            ...
            # Export graphs
            an.RunOccAccROIAnalysis.expgraph(self)
            # Export images
            an.RunOccAccROIAnalysis.expimgs(self)
